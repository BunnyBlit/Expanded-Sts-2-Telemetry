...
- `reward_taken` appears when player accepts a reward; `reward_type: "card"` has `item: null` (card ID not available at hook time — expected); `reward_type: "gold"` has `amount` field and no `item`; `reward_type: "relic"` / `"potion"` have non-null `item`
