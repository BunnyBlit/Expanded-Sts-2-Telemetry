...
| Event | Extra fields |
|-------|-------------|
| `run_start` | `game_version` (string — from `ReleaseInfoManager.Instance.ReleaseInfo?.Version`, falls back to `"dev"` for local builds), `profile` (int, 1–3 — `SaveManager.Instance.CurrentProfileId`) |
